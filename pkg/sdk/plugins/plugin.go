...
func CreateFilter(filter v1beta1.Filter, flowName string, namespace string, index int, secretLoader secret.SecretLoader) (types.Directive, error) {
	...
	case 1:
		return converters[0].ToDirective(secretLoader, fmt.Sprintf("%s_%s_%d", flowName, namespace, index))
	default:
